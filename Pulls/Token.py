...
class Token(object):
    ...
    def update_token(github_object, token_list):
        ...
        if current_token_rate_limit.search.remaining > 10 and \
        ...
        elif current_token_rate_limit.search.remaining < 10:
        ...
        token_list.sort(key=lambda x: (x.next_use_time - datetime.utcnow()).seconds, reverse=False)
        if token_list[0].get_waiting_period() > 0:
            logging.info("Going into sleep for {}".format(token_list[0].get_waiting_period()))
            time.sleep(token_list[0].get_waiting_period())
